document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule, ruleValue) {
      ...
      const messages = {
        ...
        minlength: label + ' must be at least ' + ruleValue + ' characters.',
    ...
    function validateField(field) {
      ...
      const rules = [];
      if (field.required || field.dataset.required) rules.push(['required', true]);
      if (type === 'email' || field.dataset.validate === 'email') rules.push(['email', true]);
      if (field.dataset.validate === 'phone') rules.push(['phone', true]);
      if (field.minLength && field.minLength > 0) rules.push(['minlength', field.minLength]);
      if (field.dataset.minlength) rules.push(['minlength', field.dataset.minlength]);
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        if (VALIDATORS[rule] && !VALIDATORS[rule](value, ruleValue)) {
          errorMessage = getErrorMessage(field, rule, ruleValue);
          break;
        }
      }
  ...
  if (animatableElements.length > 0) {
    animatableElements.forEach(function (el) {
      ...
      if (animClass.indexOf('fade-in-up') !== -1 || animClass.indexOf('slide-up') !== -1) {
        el.style.transform = 'translateY(30px)';
      } else if (animClass.indexOf('fade-in-down') !== -1) {
        el.style.transform = 'translateY(-30px)';
      } else if (animClass.indexOf('fade-in-left') !== -1) {
        el.style.transform = 'translateX(30px)';
      } else if (animClass.indexOf('fade-in-right') !== -1) {
        el.style.transform = 'translateX(-30px)';
      } else if (animClass.indexOf('slide-in') !== -1) {
        el.style.transform = 'translateX(30px)';
      }
    });
      ...
      const intersectionObserver = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting && entry.target.dataset.animated !== 'true') {
            const delay = entry.target.dataset.delay || entry.target.dataset.animateDelay || 0;
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מכבי תל אביב משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'c039ca6a-9ddc-475a-89fb-04a8d28077dc',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'c039ca6a-9ddc-475a-89fb-04a8d28077dc';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'c039ca6a-9ddc-475a-89fb-04a8d28077dc';
